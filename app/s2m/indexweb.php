...
$scriptUrl = $protocol . $host . str_replace('indexweb.php', 'data/channels.json', $requestUri);
...
foreach ($data['channels'] as $item) {
 ...
 if (!in_array($current['group'], $groups)) 
...
<body>
  <div class="title"><?php echo $_GET['group']?></div>
  <div class="container">
    <div class="controls">
    ...
    <?php
        ...
        <div class="channel" data-name="<?=strtolower($ch['name'])?>" data-group="<?=htmlspecialchars(strtolower($ch['group']))?>">
          ...
          <a class="watch-btn" href="live.php?id=<?=urlencode($ch['id'])?>">Watch</a>
        </div>
     <?php 
        endif;
      endforeach; 
