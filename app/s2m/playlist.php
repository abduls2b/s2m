...
function loadChannels(): ?array {
    ...
    if (!is_array($channels) || empty($channels['channels'])) {
        return null;
    }
    
    // Check if we have selected genres filter
    if (file_exists('selected.json')) {
        $selected = array_map('trim', json_decode(file_get_contents('selected.json'), true));
        if (!empty($selected)) {
            // Filter channels by selected genres
            $channels['channels'] = array_values(array_filter(
                $channels['channels'],
                function($c) use ($selected) {
                    return isset($c['genre_name']) && in_array(trim($c['genre_name']), $selected);
                }
            ));
        }
    }
